...
Deno.serve(async (req) => {
    try {
        console.log('🛒 Début du traitement des paniers abandonnés');
        ...
        for (const order of abandonedOrders) {
            try {
                ...
                // Email 1 : après 1h, si pas encore envoyé
                if (hoursSinceCreation >= 1 && reminderCount === 0) {
                    shouldSendReminder = true;
                    reminderNumber = 1;
                }
                // Email 2 : après 24h, si seulement 1 envoyé
                else if (hoursSinceCreation >= 24 && reminderCount === 1) {
                    shouldSendReminder = true;
                    reminderNumber = 2;
                }
                // Email 3 : après 72h, si seulement 2 envoyés
                else if (hoursSinceCreation >= 72 && reminderCount === 2) {
                    shouldSendReminder = true;
                    reminderNumber = 3;
                }
                ...
                console.log(`📧 Envoi email ${reminderNumber} pour commande ${order.id}`);

                // Générer l'URL de checkout (retour vers la page Commander)
                const checkoutUrl = `https://unechansonpourtoi.fr/Commander`;

                // Récupérer le template
                const template = getEmailTemplate(reminderNumber, order, checkoutUrl);
                ...
                // Envoyer copie à l'admin (BCC simulé)
                await base44.asServiceRole.integrations.Core.SendEmail({
                    from_name: 'Une Chanson Pour Toi',
                    to: 'aseckpro@gmail.com',
                    subject: `[COPIE RELANCE ${reminderNumber}] ${template.subject}`,
                    body: template.body
                });
                ...
                results.emailsSent++;
                console.log(`✅ Email ${reminderNumber} envoyé à ${order.customer_email}`);

                // Notification Telegram pour le 1er abandon uniquement
                if (reminderNumber === 1) {
                    await sendTelegramNotification(
                        `🛒 <b>Panier abandonné</b>\n\n` +
                        `👤 ${order.customer_name || 'Inconnu'}\n` +
                        `📧 ${order.customer_email}\n` +
                        `🎵 ${order.song_objective || 'Non précisé'}\n` +
                        `💰 ${order.price}€\n\n` +
                        `📧 Email de relance #1 envoyé`
                    );
                }

            } catch (orderError) {
                console.error(`❌ Erreur pour commande ${order.id}:`, orderError.message);
                results.errors.push({ orderId: order.id, error: orderError.message });
            }

            results.processed++;
        }

        console.log(`🏁 Traitement terminé: ${results.emailsSent} emails envoyés`);
